...
mat_water = gr.material({109/255, 225/255, 255/255}, {0.4, 0.4, 0.4}, 5)
...
tex = gr.texture('Assets/Textures/checkers.png')
...
sphere:scale(1, 1, 1)
sphere:translate(-1.5, 1, 0)
-- sphere:set_texture(tex, 128)
-- sphere:set_bump(bump, 100)
scene:add_child(sphere)
...
cube:translate(-4.5, 0, 0)
-- cube:set_texture(tex, 128)
-- cube:set_bump(bump, 100)
...
-- cone:set_texture(tex, 128)
-- cone:set_bump(bump, 100)
...
-- cylinder:set_texture(tex, 128)
-- cylinder:set_bump(bump, 100)
...
white_light = gr.spherical_light({0, 10, 0}, {0.7, 0.7, 0.7}, {1, 0, 0}, 2)
blue_light = gr.point_light({-5, 10, 0}, {0.7, 0.7, 0.8}, {3, 0, 0})
warm_light = gr.point_light({5, 10, 0}, {0.8, 0.7, 0.7}, {3, 0, 0})

gr.render(scene, 'demo/_softshadows.png', 512, 256,
	  {0, 6.5, 8}, {0, -6, -8}, {0, 1, 0}, 50,
