...
// The "userdata" type for a bump. Objects of this type will be
// allocated by Lua to represent bumps.
struct gr_bump_ud {
  BumpMap* bump;
};
...
// Make a Point light
extern "C"
int gr_point_light(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
extern "C"
int gr_spherical_light(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  uint radius = luaL_optnumber(L, 4, 5);

  data->light = new SphericalLight(radius);
  data->light->position = position;
  data->light->colour = colour;
  data->light->falloff[0] = falloff[0];
  data->light->falloff[1] = falloff[1];
  data->light->falloff[2] = falloff[2];
...
int gr_texture_cmd(lua_State* L)
{
  ...
  data->texture = new TextureMap(tname);
...
int gr_node_caustic_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
int gr_node_set_bump_cmd(lua_State* L)
{
  ...
  gr_bump_ud* bumpdata = (gr_bump_ud*)luaL_checkudata(L, 2, "gr.bump");
  luaL_argcheck(L, bumpdata != 0, 2, "Bump expected");

  uint scale = luaL_optnumber(L, 3, 100);

  BumpMap* bump = bumpdata->bump;

  self->setBump(bump, scale);

  return 0;
}

// Set a node's Material's Texture
extern "C"
int gr_node_set_texture_cmd(lua_State* L)
{
  ...
  TextureMap* texture = texdata->texture;

  self->setTexture(texture, scale);
...
// Set a node's Material
extern "C"
int gr_material_reflection_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"mesh", gr_mesh_cmd},
  {"point_light", gr_point_light},
  {"spherical_light", gr_spherical_light},
...
static const luaL_Reg grlib_material_methods[] = {
  {"__gc", gr_node_gc_cmd},
  {"reflection", gr_material_reflection_cmd},
  {0, 0}
};
